...
nc = ''
'#!/usr/bin/env python#
Made using PyLock by ***REMOVED*** https: //github.com/NDev/Python-Script-Locker
  import sys
def exitmsg(msg):
  print(msg)
...
print(ex)
''
' % (salt, Fernet(base64.urlsafe_b64encode(kdf.derive(getpass.getpass("Password to use: ")))).encrypt(script))
try:
f = open(sloc, "w+")
f.write(nc)
except IOError:
